...
<font color='red'>注意是在v-model上的</font>
...
- .trim  自动过滤用户输入的首尾空白字符
  ...
  - 只能去掉首尾的 <font color='red'>不能去除中间的空格</font>

- .lazy   将input事件切换成change事件

  - .lazy 修饰符延迟了同步更新属性值的时机。即将原本绑定在 input 事件的同步逻辑转变为绑定在<font color='red'> change 失去焦点</font>事件上
- 在失去焦点 或者 按下回车键时才更新
...
## 3.2 Vue.directive  注册全局指令 带参数

binding指的就是绑定的msg

```html
  <input type="text" v-colo='msg'>
  或者<input type="text" v-colo='{color="blue"}'>
或者
  ...
  <script type="text/javascript">
        ...
        var vm = new Vue({
            ...
            data: {
             ...
             //watch  属性 定义 和 data 以及 methods 平级 
            watch: {
...
- <font color='red'>全局注册时是filter，没有s的。而局部过滤器是filters，是有s的</font>
