...
Maven的依赖管理，分模块开发与设计，聚合，继承，属性，版本管理，资源配置，多环境开发配置，跳过测试，私服



# 2021.04.09

分布式系统概念，Dubbo概述、入门、高级特性



# 2021.04.11

ZooKeeper命令操作，JavaAPI操作，分布式锁，集群搭建，角色
...
SpringBoot入门工程，特点包括依赖管理和自动配置，一些注解：
...
1. FastDFS的架构(包含3个部分)
2. 请求、上传、下载、访问图片的流程
3. 将图片模块单独提取出来，改造成微服务
4. 图片上传代码
5. 还有公共接口的的设计与思想(涉及泛型、接口、抽象类、反射知识点)



# 2021.10.10

商品发布，包括商品分类查询、品牌查询、规则查询、参数查询、添加数据，回显与修改数据，雪花算法，spu，sku，代码生成器
...
1. 单点登录流程(前一天的用户登录也算单点登录，只不过认证服务器是放在网关模块，利用过滤器实现的；除此之外，也可将认证服务器独立成一个微服务，和网关分开)
2. 使用Oauth2.0技术进行第三方登录的案例流程
3. 公钥加密，私钥解密(加密/解密)；私钥加密，公钥校验(签名，防止篡改)
4. 解决方案：spring cloud security+jwt+oauth2.0+RSA算法实现畅购的单点登录和授权
5. 畅购主要做了单点登录(生成令牌)，以及登录后微服务之间内部互相访问的权限控制



# 2021.11.21

1. 配置资源服务器：添加依赖，配置资源服务器Configuration类，添加公钥
2. 改造网关：网关只检查有没有令牌，并不校验；网关将令牌传递到下游资源微服务(本案例令牌是cookie方式，如果令牌在header中，默认会传递下去)，具体由各个资源微服务校验
3. 优化登录流程：客户端和用户的动态数据获取。注意登录时远程Feign调用获取用户名称和密码校验时，要将对应的请求路径放行，即不校验公钥
4. 权限校验，加注解即可，区分不同用户拥有的访问资源权限
5. 内部微服务相互访问、远程调用时，也需要携带令牌(Feign拦截器实现)



# 2021.11.22

登录跳转，购物车下单：库存、积分、清空购物车，分布式事务回滚
...
Redis设计与实现：Redis底层数据结构，RDB与AOF持久化，主从复制，Sentinel模式和集群模式
